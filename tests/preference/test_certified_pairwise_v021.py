...
class _ScriptedGateway:
    ...
    async def judge(self, *, system, user, prompt_template_version, schema_hash,
        ...
        self.calls.append({"system": system, "user": user,
                           ...
                           "schema_hash": schema_hash, "stage": stage})
        return {"content": self.payloads.pop(0)}


def _payload(first_q: float, second_q: float, preferred: str,
             defects_first=None, defects_second=None, confidence: float = 0.9) -> str:
    return json.dumps({
        "first_quality": first_q, "second_quality": second_q,
        "preferred": preferred,
...
class TestCertifiedPairwise:
    ...
    def test_valid_pair_with_real_defect_and_consistent_orders(self):
        ...
        j = self._judge(gw)
        out = _run(j.judge_pair(prompt=PROMPT, chosen_text=CHOSEN,
    ...
    def test_two_calls_made_with_reversed_order(self):
        ...
        j = self._judge(gw)
        _run(j.judge_pair(prompt=PROMPT, chosen_text=CHOSEN,
                          rejected_text=REJECTED_BAD_NUMBER, evidence_text=EVIDENCE))
    ...
    def test_length_signature_detected(self):
        ...
        j = self._judge(gw)
        out = _run(j.judge_pair(prompt=PROMPT, chosen_text=CHOSEN,
                                rejected_text=verbose_rejected, evidence_text=EVIDENCE,
                                force_chosen_first=True))
    ...
    def test_both_bad_is_invalid(self):
        ...
        j = self._judge(gw)
        out = _run(j.judge_pair(prompt=PROMPT, chosen_text="Wrong wrong wrong.",
                                rejected_text="Also wrong.", evidence_text=EVIDENCE,
                                force_chosen_first=True))
    ...
    def test_chosen_below_absolute_floor_invalid(self):
        ...
        j = self._judge(gw)
        out = _run(j.judge_pair(prompt=PROMPT, chosen_text="Garbage.",
                                rejected_text="Worse garbage.", evidence_text=EVIDENCE,
                                force_chosen_first=True))
        assert out.verdict == "invalid"
    ...
    def test_empty_side_no_signal(self):
        ...
        j = self._judge(gw)
        out = _run(j.judge_pair(prompt=PROMPT, chosen_text=CHOSEN,
                                rejected_text="", evidence_text=EVIDENCE))
        assert out.verdict == "review"
...
class TestNoAutoPerfectSignal:
    ...
    def test_to_dict_has_full_lineage_fields(self):
        required = {
            "verdict", "preference_margin", "order_consistent",
            "chosen_absolute", "rejected_absolute", "rejected_defect_codes",
            "judge_identity", "prompt_template_version", "schema_hash",
            "confidence", "reason_codes", "rationale",
        }
...
class TestPreferenceValidatorIntegration:
    def _example(self):
        ...
        ex = TrainingExample(
            ...
            rejected_messages=[
                CanonicalMessage(role="assistant", content=REJECTED_BAD_NUMBER)
            ],
    ...
    def test_certified_profile_demotes_on_order_disagreement(self):
        ...
        v = PreferenceValidator(preference_profile="certified-pairwise",
                                gateway=gw, judge=CertifiedPairwiseJudge(
                                    gw, model="pref-judge/1b"))
