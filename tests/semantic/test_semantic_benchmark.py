...
CASES: list[tuple[str, str, str, str]] = [
    ...
    ("causal_reversal",
     ...
     "The thermal fuse blowing causes overheating.", "contradicted"),
    ("entity_role_reversal",
     "The warehouse ships sensors to the assembly plant.",
     "The assembly plant ships sensors to the warehouse.", "contradicted"),
    ("negation",
     "Calibration is required before first use.",
     "Calibration is not required before first use.", "contradicted"),
    ("number_change",
     "The battery lasts 12 hours on a full charge.",
     "The battery lasts 20 hours on a full charge.", "contradicted"),
    ("date_change",
     "Warranty claims must be filed by January 15, 2025.",
     "Warranty claims must be filed by February 15, 2025.", "contradicted"),
    ("unit_conversion",
     "The cable is 2 meters long.",
     "The cable is 200 centimeters long.", "entailed"),
    ("unit_error",
     "The tank holds 50 liters.",
     "The tank holds 50 milliliters.", "contradicted"),
    ("percentage_base",
     "Sales rose from 100 to 150 units, a 50 percent increase.",
     "Sales rose 150 percent to 150 units.", "contradicted"),
    ("unsupported_synthesis",
     "The XR-9 sensor passed calibration.",
     "The XR-9 sensor passed calibration and received international "
     "certification in Geneva.", "contradicted"),
    ("incomplete_enum_wrong_total",
     "The kit contains a base plate, a lid, and four screws.",
     "The kit contains exactly three items: a base plate, a lid, and four "
     "screws.", "unverified"),
    # --- entailment classes -------------------------------------------------
    ("paraphrase_entailment",
     "Each unit undergoes inspection for cracks before it leaves the plant.",
     "Units are checked for cracks prior to leaving the factory.", "entailed"),
    ("multi_sentence_evidence",
     "The firmware update ships on March 3. Devices updated after that date "
     "gain the new scheduler.",
     "Devices gain the new scheduler after updating firmware released "
     "March 3 or later.", "entailed"),
    # --- distractors / conflicts -------------------------------------------
    ("distractor_spans",
     "Model A costs 300 euros. Model B costs 450 euros and includes a stand.",
     "Model B costs 300 euros.", "contradicted"),
    ("cross_document_conflict",
     ...
     "kilograms.",
     "The current payload limit is 10 kilograms.", "unverified"),
]
...
class TestDeterministicBenchmark:
    ...
    def results(self):
        ...
        for category, evidence, answer, expected in CASES:
            ...
            if ClaimVerdict.contradicted in got:
                ...
                observed = "unverified"
            out.append({
                "category": category, "expected": expected, "observed": observed,
            })
        return out
    ...
    def test_all_twelve_categories_present(self):
        ...
        required = {
            "causal_reversal", "entity_role_reversal", "negation", "date_change",
            "number_change", "unit_conversion", "unit_error", "percentage_base",
            "incomplete_enum_wrong_total", "unsupported_synthesis",
            "paraphrase_entailment", "multi_sentence_evidence", "distractor_spans",
            "cross_document_conflict",
        }
    ...
    def test_rates_published_with_wilson_intervals(self, results):
        ...
        assert fa_rate == 0.0, (
            "deterministic gate must never accept an expected contradiction"
        )
        assert fr_rate <= 0.34, (
