...
class TestBlockLocation:
    def test_docling_prov_shape_becomes_canonical_location(self):
        block = {
            ...
            "prov": [
                {"page_no": 3, "bbox": {"l": 10.0, "t": 20.0, "r": 110.0, "b": 40.0,
    ...
    def test_merge_unions_pages_and_dedups_refs(self):
        units = [
            type("U", (), {"location": {"pages": [1, 2], "bboxes": [{"a": 1}],
                                         "element_refs": ["docling#/main_text/0"]}})(),
            type("U", (), {"location": {"pages": [2, 3], "bboxes": [],
                                        "element_refs": ["docling#/main_text/0",
                                                         "docling#/main_text/1"]}})(),
        ]
...
class TestChunkerCarriesLocation:
    def test_docling_blocks_produce_located_chunks(self):
        docling = {
            "main_text": [
                {"text": "Widgets", "label": "title",
                 "prov": [{"page_no": 1, "bbox": {"l": 0, "t": 0, "r": 200, "b": 20,
                                                  "coord_origin": "TOPLEFT"}}]},
                {"text": "A widget is a base plate plus a lid.", "label": "text",
                 "prov": [{"page_no": 1, "bbox": {"l": 0, "t": 30, "r": 200, "b": 60,
                                                  "coord_origin": "TOPLEFT"}}]},
                {"text": "Torque the lid to 5 N·m.", "label": "text",
                 "prov": [{"page_no": 2, "bbox": {"l": 0, "t": 70, "r": 200, "b": 100,
                                                  "coord_origin": "TOPLEFT"}}]},
            ]
        }
        chunks = chunk_document(docling, ChunkCfg(target_tokens=50, min_tokens=5,
                                                  max_tokens=300))
        assert chunks
    ...
    def test_markdown_fallback_has_empty_location_not_fake_pages(self):
        canonical = {
            ...
            "blocks": [
                ...
                {"type": "paragraph", "text": "A widget is a base plate plus a lid.",
                 "heading_path": ["Assembly"]},
            ],
        }
        chunks = chunk_document(canonical, ChunkCfg(target_tokens=50, min_tokens=5,
                                                    max_tokens=300))
        assert chunks
...
class TestServiceSpans:
    ...
    def test_spans_from_docling_input_carry_derived_precision(self):
        ...
        docling = {
            "main_text": [
                {"text": "Report", "label": "title",
                 "prov": [{"page_no": 1, "bbox": {"l": 0, "t": 0, "r": 100, "b": 10,
                                                  "coord_origin": "TOPLEFT"}}]},
                {"text": "Findings are material.", "label": "text",
                 "prov": [{"page_no": 1, "bbox": {"l": 0, "t": 20, "r": 100, "b": 50,
                                                  "coord_origin": "TOPLEFT"}}]},
            ]
        }
    ...
    def test_spans_from_markdown_are_honest_about_precision(self):
        ...
        canonical = {
            ...
            "blocks": [
                ...
                {"type": "paragraph", "text": "Each unit is inspected for cracks.",
    ...
    def test_multipage_span_reports_range_not_single_page(self):
        ...
        if len(chunks) == 1:
            ...
            assert all(sp.precision in (SpanPrecision.exact_bbox, SpanPrecision.exact_page)
                       for sp in spans)
