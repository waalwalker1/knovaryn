...
from ...domain.errors import BudgetExceededError, JobStateError, KnovarynError
...
from .retry import backoff_delay, is_retryable, RetryPolicy
...
class JobEvents(Protocol):
    ...
    async def get_events(self, job_id: str, *, cursor: int | None = None, limit: int = 100) -> tuple[list[JobEvent], int | None]: ...
...
class StageContext:
    ...
    async def event(self, message: str, *, level: str = "info", event_type: str = "log", stage: str = "", payload: dict[str, Any] | None = None) -> None:
        ev = JobEvent(
            id=self.ids.new(),
            job_id=self.job.id,
    ...
    async def checkpoint(self, stage: str, *, step: int = 0, key: str, value: dict[str, Any]) -> None:
...
class JobEngine:
    ...
    def __init__(self, *, ids: IdGenerator, repo: JobEvents, retry_policy: RetryPolicy | None = None) -> None:
        self._ids = ids
        self._repo = repo
        self._retry_policy = retry_policy or RetryPolicy()

    async def run(self, job: Job, stages: list[tuple[str, StageFn]], *, services: dict[str, Any] | None = None) -> Job:
        ...
        try:
            ...
            for idx, (name, fn) in enumerate(stages):
                if ctx.cancellation_requested():
                    break
                if ctx.extras.get("completed_stages", set()) and name in ctx.extras["completed_stages"]:
    ...
    async def _execute_with_retry(self, ctx: StageContext, name: str, fn: StageFn) -> None:
        attempt = 0
        while True:
            try:
                ...
                if not is_retryable(exc, max_attempts=self._retry_policy.max_attempts, attempt=attempt):
                    raise
                await ctx.event(f"stage {name} retrying ({attempt+1})", level="warning", stage=name)
                attempt += 1
